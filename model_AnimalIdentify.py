# -*- coding: utf-8 -*-
# @Time : 2024/07/17/0017 12:01
# @Author : rainbow
# @Location: hennapuyang
# @File : model.py
...
class Unet(nn.Module):
    def __init__(self,in_channel):
        ...
        self.upsampling1 = Upsampling(in_channels=512, out_channels=256, upsampling_size=(2, 2))
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.resnet = ResNet(in_channel=3,num_resnet_block= 6)
        self.unet  = Unet(in_channel=3)
...
if __name__ =='__main__':
    ...
    # model = resNet(in_channel=3).to(device)
    # model = Unet(in_channel=3).to(device)
    # model = CNN(in_channel=128).to(device)
    model = Net().to(device)
    summary(model,input_size=(3,224,224))
    pred = model(x)
    print(np.max(pred.cpu().detach().numpy()[0]))
